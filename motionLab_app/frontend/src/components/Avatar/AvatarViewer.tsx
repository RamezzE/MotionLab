...
const AvatarViewer: React.FC<AvatarViewerProps> = ({
    ...
    const containerClasses = displayMode === 'list'
        ? 'flex-row gap-x-4 max-w-[300px] hover:border-4 hover:border-blue-500 transition-all duration-300 relative'  // Horizontal layout for list mode
        : 'flex-col justify-center w-full h-[50vh] xs:flex-row lg:flex-row'; // Flex-col for xs, flex-row for larger screens

    return (
        <div className={`flex bg-black/50 px-4 rounded-xl items-center hover:pointer ${containerClasses}`}>
            ...
            {/* Edit and Delete Icons for list view */}
            {displayMode === 'list' && (
